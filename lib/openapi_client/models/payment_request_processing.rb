...
module OpenapiClient
  ...
  class PaymentRequestProcessing
    # Indicates whether the payment is an [Account Funding Transaction](https://docs.checkout.com/payment-actions/account-funding-transactions)
    attr_accessor :aft

    attr_accessor :dlocal
    ...
    def self.openapi_types
      {
        :'aft' => :'Boolean',
        :'dlocal' => :'PaymentRequestProcessingDlocal'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentRequestProcessing` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentRequestProcessing`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'aft')
        self.aft = attributes[:'aft']
      end

      if attributes.key?(:'dlocal')
        self.dlocal = attributes[:'dlocal']
      end
    end
