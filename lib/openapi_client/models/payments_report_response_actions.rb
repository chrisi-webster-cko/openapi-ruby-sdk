...
module OpenapiClient
  class PaymentsReportResponseActions
    # The type of action administered to a payment
    attr_accessor :type

    # The unique identifier associated with an action (referred to as a charge ID in the Hub)
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'breakdown' => :'Array<PaymentsReportResponseBreakdown>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentsReportResponseActions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentsReportResponseActions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'response_description')
        self.response_description = attributes[:'response_description']
      end
    ...
    def hash
      [type, id, processed_on, response_code, response_description, breakdown].hash
    end
