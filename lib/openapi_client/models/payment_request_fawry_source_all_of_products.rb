...
module OpenapiClient
  class PaymentRequestFawrySourceAllOfProducts
    ...
    # The price of the item.  Expressed using Checkout.com's standard rules for calculating payment values.
    attr_accessor :price
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentRequestFawrySourceAllOfProducts` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentRequestFawrySourceAllOfProducts`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @product_id.to_s.length > 265
        invalid_properties.push('invalid value for "product_id", the character length must be smaller than or equal to 265.')
      end
    ...
    def valid?
      ...
      return false if @product_id.to_s.length > 265
    ...
    def product_id=(product_id)
      ...
      if product_id.to_s.length > 265
        fail ArgumentError, 'invalid value for "product_id", the character length must be smaller than or equal to 265.'
      end
    ...
    def hash
      [product_id, quantity, price, description].hash
    end
