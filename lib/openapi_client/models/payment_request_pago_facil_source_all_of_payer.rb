...
module OpenapiClient
  # Payer object
  class PaymentRequestPagoFacilSourceAllOfPayer
    # Payer's full name.
    attr_accessor :name
    ...
    # Payer's document identifier in Argentina, namely, Documento Nacional de Identidad (DNI) or Clave Ãšnica de IdentificaciÃ³n Tributaria (CUIT).
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentRequestPagoFacilSourceAllOfPayer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentRequestPagoFacilSourceAllOfPayer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @document.to_s.length > 100
        invalid_properties.push('invalid value for "document", the character length must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @document.to_s.length > 100
      true
    end
    ...
    def document=(document)
      ...
      if document.to_s.length > 100
        fail ArgumentError, 'invalid value for "document", the character length must be smaller than or equal to 100.'
      end
