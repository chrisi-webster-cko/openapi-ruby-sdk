...
module OpenapiClient
  # PagoFacil Source
  class PaymentResponsePagoFacilSource < PaymentResponseSource
    ...
    # The type of the integration:   - `direct`: a ticket object is returned, which a merchant can use to render a ticket to a customer; no redirect url is returned   - `redirect`: a redirect url is returned, which the merchant redirects the customer to, to continue payment
    attr_accessor :integration_type

    # Payment description
    attr_accessor :description

    # Ticket object. Returned only when the payment was submitted with integration_type as 'direct'.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentResponsePagoFacilSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentResponsePagoFacilSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      integration_type_validator = EnumAttributeValidator.new('String', ["direct", "redirect"])
    ...
    def integration_type=(integration_type)
      validator = EnumAttributeValidator.new('String', ["direct", "redirect"])
    ...
    def hash
      [dlocal_order_id, dlocal_payment_id, failure_reason, failure_code, integration_type, description, ticket].hash
    end
