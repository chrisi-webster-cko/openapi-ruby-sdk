  ...
  class InstrumentRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InstrumentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InstrumentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^(tok)_(\w{26})$ | ^(card_tok)_(\w{12})$/)
      if @token !~ pattern
    ...
    def valid?
      ...
      return false if @type !~ Regexp.new(/token/)
      return false if @token.nil?
      return false if @token !~ Regexp.new(/^(tok)_(\w{26})$ | ^(card_tok)_(\w{12})$/)
      true
    end
