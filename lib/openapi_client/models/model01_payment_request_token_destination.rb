...
module OpenapiClient
  # A token payment destination
  class Model01PaymentRequestTokenDestination < PaymentRequestDestination
    # The Checkout.com token (e.g., a card, wallet or token)
    attr_accessor :token
    ...
    # The payout destination owner's billing address. This will override the billing address specified during tokenization
    attr_accessor :billing_address

    # The payout destination owner's phone number. This will override the phone number specified during tokenization
    attr_accessor :phone
    ...
    def self.openapi_all_of
      [
      :'01PaymentRequestTokenDestinationAllOf',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Model01PaymentRequestTokenDestination` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Model01PaymentRequestTokenDestination`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @token !~ Regexp.new(/^(tok)_(\w{26})$/)
    ...
    def hash
      [token, first_name, last_name, billing_address, phone].hash
    end
