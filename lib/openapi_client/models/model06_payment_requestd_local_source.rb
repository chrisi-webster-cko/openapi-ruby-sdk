...
module OpenapiClient
  ...
  class Model06PaymentRequestdLocalSource < PaymentRequestSource
    # The payment source type <br> *Note:* *To make a payment with full card details, you must be <a href=\"https://docs.checkout.com/risk-management/pci-compliance\" target=\"blank\">SAQ D PCI compliant</a>*
    attr_accessor :type

    # The LATAM local card scheme card number (without separators)
    ...
    def self.openapi_all_of
      [
      :'06PaymentRequestdLocalSourceAllOf',
      :'Model03PaymentRequestCardSource',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Model06PaymentRequestdLocalSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Model06PaymentRequestdLocalSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [type, number, expiry_month, expiry_year, name, cvv, stored, billing_address, phone].hash
    end
