...
module OpenapiClient
  class InstrumentResponse
    # The unique identifier of the payment source or destination that can be used later for payments
    attr_accessor :id

    # The underlying instrument type (for instruments created from Checkout.com tokens, this will reflect the type of instrument that was tokenized)
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InstrumentResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InstrumentResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, type, expiry_month, expiry_year, scheme, last4, bin, card_type, card_category, issuer, issuer_country, product_id, product_type].hash
    end
