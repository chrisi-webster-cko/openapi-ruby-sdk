...
module OpenapiClient
  # The links related to the SEPA payment source
  class 01SepaSourceResponseAllOfLinks
    # The URI of the payment source
    attr_accessor :_self

    # A link to cancel the SEPA Direct Debit mandate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::01SepaSourceResponseAllOfLinks` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::01SepaSourceResponseAllOfLinks`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
