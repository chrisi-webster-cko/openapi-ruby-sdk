...
module OpenapiClient
  class Model3dsData
    ...
    # Specifies an exemption reason so that the payment is not processed using 3D Secure authentication
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Model3dsData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Model3dsData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'downgraded')
        self.downgraded = attributes[:'downgraded']
      end
      ...
      if attributes.key?(:'signature_valid')
        self.signature_valid = attributes[:'signature_valid']
      end
    ...
    def hash
      [downgraded, enrolled, signature_valid, authentication_response, cryptogram, xid, version, exemption].hash
    end
