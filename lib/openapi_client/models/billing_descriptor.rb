  ...
  class BillingDescriptor
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BillingDescriptor` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BillingDescriptor`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @city.to_s.length > 13
        invalid_properties.push('invalid value for "city", the character length must be smaller than or equal to 13.')
      end
    ...
    def valid?
      ...
      return false if @city.to_s.length > 13
      true
    end
    ...
    def city=(city)
      ...
      if city.to_s.length > 13
        fail ArgumentError, 'invalid value for "city", the character length must be smaller than or equal to 13.'
      end
