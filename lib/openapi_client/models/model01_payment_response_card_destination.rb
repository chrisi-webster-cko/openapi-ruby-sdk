...
module OpenapiClient
  # A card payment destination
  class Model01PaymentResponseCardDestination < PaymentResponseDestination
    # The expiry month
    ...
    def self.openapi_all_of
      [
      :'01PaymentResponseCardDestinationAllOf',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Model01PaymentResponseCardDestination` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Model01PaymentResponseCardDestination`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [expiry_month, expiry_year, name, scheme, last4, fingerprint, bin, card_type, card_category, issuer, issuer_country, product_id, product_type].hash
    end
