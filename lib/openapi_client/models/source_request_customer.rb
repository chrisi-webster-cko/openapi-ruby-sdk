...
module OpenapiClient
  ...
  class SourceRequestCustomer
    # The identifier of an existing customer. If neither customer `id` or `email` is provided, then  a new customer will be registered 
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SourceRequestCustomer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SourceRequestCustomer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      return false if !@id.nil? && @id !~ Regexp.new(/^(cus)_(\w{26})$/)
      true
    end
