...
module OpenapiClient
  # Information about the recipent of the payment's funds. Relevant for both [Account Funding Transactions](https://docs.checkout.com/payment-actions/account-funding-transactions) and VISA or MasterCard [domestic UK transactions processed by Financial Institutions](https://docs.checkout.com/risk-management/requirements-for-financial-institutions).
  class PaymentRecipient
    # The recipient's date of birth (yyyy-mm-dd)
    attr_accessor :dob

    # The first six digits and the last four digits of the primary recipient's card, without spaces, or up to ten digits of the primary recipient's account number
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentRecipient` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentRecipient`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@dob.nil? && @dob.to_s.length > 10
        invalid_properties.push('invalid value for "dob", the character length must be smaller than or equal to 10.')
      end
      ...
      if !@account_number.nil? && @account_number.to_s.length < 10
        invalid_properties.push('invalid value for "account_number", the character length must be great than or equal to 10.')
      end
    ...
    def valid?
      return false if !@dob.nil? && @dob.to_s.length > 10
    ...
    def dob=(dob)
      if !dob.nil? && dob.to_s.length > 10
        fail ArgumentError, 'invalid value for "dob", the character length must be smaller than or equal to 10.'
      end
    ...
    def account_number=(account_number)
      ...
      if !account_number.nil? && account_number.to_s.length < 10
        fail ArgumentError, 'invalid value for "account_number", the character length must be great than or equal to 10.'
      end
    ...
    def hash
      [dob, account_number, zip, first_name, last_name].hash
    end
